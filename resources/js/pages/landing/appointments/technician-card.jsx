...
const AppointmentTechnicianCard = ({ technician }) => {
  return (
    <div className="flex items-center rounded-md border p-4">
      {/* Avatar */}
      <Avatar className="border-2 h-14 w-14 lg:ml-4 lg:mr-4 lg:h-24 lg:w-24">
        {technician.image ? (
          <>
            <AvatarImage
      ...
      <div className="ml-4 truncate overflow-hidden">
        <h3 className="text-md font-semibold line-clamp">{technician.name}</h3>
        <div>
          {technician.specialists.length > 0 ? (
            technician.specialists.slice(0, 3).map((specialist, index) => (
              <span key={specialist.id} className="mr-1 inline-block text-xs">
                {index > 0 && ", "}
                {specialist.name}
