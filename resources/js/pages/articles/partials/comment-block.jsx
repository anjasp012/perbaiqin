...
import { CommentOptions } from '@/pages/articles/partials/comment-options';
                    ...
                    {comments.map((comment) => (
                        <div className="flex" key={comment.id}>
                            <div className="mr-4 flex-shrink-0">
                                <img className="h-10 w-10 rounded-full" src={comment.author.gravatar} alt={comment.author.name} />
                            </div>
                            <div className="w-full">
                                <div className="flex items-center justify-between">
                                    <h4 className="font-semibold">
                                        {comment.author.name}
                                        <small className="ml-2 text-sm text-muted-foreground">{comment.created_at}</small>
                                    </h4>
                                    {auth.user && <CommentOptions {...{ auth, article, comment }} />}
                                </div>
                                <div className="mt-1 leading-relaxed text-muted-foreground">
                                    <div
                                        className="prose prose-sky dark:prose-invert prose-pre:bg-accent"
                                    ...
                                    <div className="mt-4 flex items-center gap-x-4 py-2">
                                        {comment.can_be_replied && (
                                            <button
                                                ...
                                                className="flex h-6 w-6 items-center text-gray-500 hover:text-foreground focus:outline-none"
                                            >
                                                <ChatBubbleOvalLeftIcon className="h-4 w-4 shrink-0" />
                                                {comment.children_count > 0 && <span className="ml-2 text-sm">{comment.children_count}</span>}
                                            </button>
                                        )}
                                        <Link
                                            as="button"
                                        ...
                                        >
                                            ...
                                            {comment.likes_count > 0 && <span className="ml-2 text-sm">{comment.likes_count}</span>}
                                        </Link>
                                        ...
                                        {auth.user?.id === comment.author.id && (
                                            <button
                                                ...
                                                className="inline-flex h-6 w-6 items-center text-gray-500 hover:text-foreground focus:outline-none"
                                            >
