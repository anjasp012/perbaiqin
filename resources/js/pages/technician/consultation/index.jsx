                ...
                <TableBody>
                  {consultations.data.map((consultation, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell><Badge>{consultation.status}</Badge></TableCell>
                      <TableCell>{formatDateTime(consultation.created_at)}</TableCell>
                      <TableCell>
                        <div className="flex gap-2">
                          <Link href={route('technician.consultations.chat', consultation.id)} className={buttonVariants({ size:'sm', variant: 'secondary' })}>
                            Go to Chat
                          </Link>
                          <Button
                            size="sm"
